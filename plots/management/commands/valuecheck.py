...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		def getSummary(listofframes,debts_df_list):
		    x = 1
		    TI,TV,Summary = pd.DataFrame(),pd.DataFrame(),pd.DataFrame()
		    for chucko in listofframes:
		        TI = TI.join(chucko[['Total Investment']],how = 'outer',rsuffix = str(x)).sort_index()
		        TV = TV.join(chucko[['Total Value']],how = 'outer',rsuffix = str(x)).sort_index()
		        x += 1
		    for chucko in debts_df_list:
		    	TI = TI.join(chucko[['Amount Invested']],how = 'outer',rsuffix = str(x+1)).sort_index()
		    	TV = TV.join(chucko[['Total Value']],how = 'outer',rsuffix = str(x+1)).sort_index()
		    	TI.fillna(method='ffill',inplace=True)
		    	TV.fillna(method='ffill',inplace=True)
		    	print('***************************************')
		    	print(TI)
		    	x += 1
